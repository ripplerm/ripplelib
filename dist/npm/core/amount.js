...
var XRPValue = _require.XRPValue;
...
var MIN_IOU_VALUE = new IOUValue(consts.min_value);
...
Amount.from_components_unsafe = function (value, currency, issuer, isNative) {
  var result = new Amount(value);
  result._is_native = isNative;
  result._currency = currency;
  result._issuer = issuer;

  result._value = value.isZero() && value.isNegative() ? value.negate() : value;
  return result;
};
...
Amount.prototype.add = function (addend) {
  var addendAmount = addend instanceof Amount ? addend : Amount.from_json(addend);
...
Amount.prototype.subtract = function (subtrahend) {
  ...
  var subsAmount = subtrahend instanceof Amount ? subtrahend : Amount.from_json(subtrahend);
  return this.add(subsAmount.negate());
};

// XXX Diverges from cpp.
Amount.prototype.multiply = function (multiplicand) {
  var multiplicandValue = multiplicand instanceof Amount ? multiplicand._value : Amount.from_json(multiplicand)._value;
...
Amount.prototype.divide = function (divisor) {
  var divisorValue = divisor instanceof Amount ? divisor._value : Amount.from_json(divisor)._value;
...
Amount.prototype.compareTo = function (to) {
  var toAmount = to instanceof Amount ? to : Amount.from_json(to);
...
Amount.prototype.parse_quality = function (quality, counterCurrency, counterIssuer, opts) {
  ...
  */
  ...
  if (this._is_native) {
    this._set_value(new XRPValue(nativeAdjusted.round(6, XRPValue.getBNRoundDown()).toString()));
  } else {
...
Amount.prototype.parse_value = function (j) {
  this._is_native = false;
  var newValue = new IOUValue(j, IOUValue.getBNRoundDown());
  this._set_value(newValue);
  return this;
};
...
Amount.prototype.to_text = function () {
  ...
  var mantissa = utils.getMantissa16FromString(absValue(this._value).toString());
