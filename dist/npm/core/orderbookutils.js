...
function createAmount(value, currency_, counterparty_) {
  ...
  var currency = currency_ instanceof Currency ? currency_ : Currency.from_json(currency_);

  var counterparty = counterparty_ instanceof UInt160 ? counterparty_ : UInt160.from_json(counterparty_);
...
OrderBookUtils.getOfferTakerGetsFunded = function (offer, currency_, issuer_) {
  ...
  var currency = currency_ || getCurrencyFromOffer(offer);
  var issuer = issuer_ || getIssuerFromOffer(offer);
...
OrderBookUtils.getOfferTakerGets = function (offer, currency_, issuer_) {
  ...
  var currency = currency_ || offer.TakerPays.currency;
  var issuer = issuer_ || offer.TakerPays.issuer;
...
OrderBookUtils.getOfferQuality = function (offer, currencyGets, currency_, issuer_) {
...
OrderBookUtils.normalizeAmount = function (value) {
  return Amount.from_components_unsafe(new IOUValue(value), OrderBookUtils.CURRENCY_ONE, OrderBookUtils.ISSUER_ONE, false);
};
