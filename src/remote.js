...
Remote.prototype.requestRippleBalance = function (options_, callback_) {
  ...
  function rippleState(message) {
    ...
    request.emit('ripple_state', {
      account_balance: (accountHigh ? balance.negate() : balance.clone()).parse_issuer(options.issuer),
      peer_balance: (!accountHigh ? balance.negate() : balance.clone()).parse_issuer(options.account),
...
Remote.prototype.requestGatewayBalances = function(options, callback) {
  ...
  if (!lodash.isUndefined(options.hotwallet)) {
